...
/*
	Global var
*/
static void *g_for_flex_swap = NULL;
...
void QuickSort(void *arr, size_t size, size_t elem_size, cmp_func_t cmp)
{
	...
	g_for_flex_swap = malloc(elem_size);
	if (!g_for_flex_swap)
	{
		return ;
	}
	...
	free(g_for_flex_swap);
	g_for_flex_swap = NULL;
}
...
int FlexibleSwap(void *a, void *b, size_t size_of_elem)
{
	...
	memcpy(g_for_flex_swap, a, size_of_elem);
	memcpy(a, b, size_of_elem);
	memcpy(b, g_for_flex_swap, size_of_elem);
...
void RecQuickSort4(quick_sort_t *handel, int start, int end, int i)
{
	int pivot_index = 0;
	...
	if (start < end)
	{
		...
		if (i)
		{
		RecQuickSort4(handel, start, pivot_index - 1, i - 1); /* Recursive to the left */
		RecQuickSort4(handel, pivot_index + 1, end, i - 1); /* Recursive to the right */
		}
	}
}
...
void QuickSort4(void *arr, size_t size, size_t elem_size, cmp_func_t cmp)
{
	...
	RecQuickSort4(&handel, 0, size - 1, 0);
