...
static void *Handler(void *dest)
{
	unique_id_t *tmp = dest;
...
void TestMTCreate()
{
	...
	size_t dump = 0;
	unique_id_t tmp = {0};
	unique_id_t ret[THREADS] = {0};

	for (i = 0; i < THREADS; ++i)
	{
		pthread_create(&thread[i], NULL, &Handler, &ret[i]);
	}

	for (i = 0; i < THREADS; ++i)
	{
		pthread_join(thread[i], (void**)&dump);
	}

	for (i = 0; i < THREADS; ++i)
	{
	//	PrintUid(ret[i]);
		for (a = i + 1; a < THREADS; ++a)
		{
			if (UidIsSame(ret[i], ret[a]))
			{
				ReturnComper(0, 1);
				return ;
			}
