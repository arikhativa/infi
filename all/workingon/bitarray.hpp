...
typename BitArray<Size>::BitProxy BitArray<Size>::operator[] (size_t index)
{
    if (Size <= index)
    {
        ...
        return BitArray<Size>::BitProxy(nul, size_t(0)); // Error
    }
...
void BitArray<Size>::SetBit(size_t index, bool value)
{
    ...
    m_vec[tmp.m_arr_index] &= ~(one << tmp.m_bit_index);
    one = value;
    m_vec[tmp.m_arr_index] |= one << tmp.m_bit_index;
}
...
// ******************************* Fanctors ***********************************
FunctorCount::FunctorCount(size_t& ret) : m_count(ret)
{}
